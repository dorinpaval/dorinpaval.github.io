...
async function saveTw(event) {
    ...
    console.log("Is working",document.getElementById('upload'))
    if (content.length > 0) {
        let youtube = '';
...
async function createTweets() {
    ...
    <div class="post_body">
        ...
        <div class="post_footer" style="width:78%" >
            ...
            <div><span class="material-icons">favorite_border </span>${element.likes}</div>
    ...
    <div class="post_body">
        ...
        if (oldContent.includes(loading)) {
            document.getElementById("tweets").innerHTML = oldContent.replace(loading, htmlfinal) + loading;
        }
        else { document.getElementById("tweets").innerHTML = htmlfinal + loading };
        //alert(document.getElementById("tweets").innerHTML)
    }
    else {
...
function showPreview(event) {
    if (event.target.files.length > 0) {
            ...
            console.log("display is " , document.getElementById("hidePreview").style.display)
...
function closePreview(){
        ...
        clk.setAttribute("data-clicked", "true");
...
async function deleteUser(){
    let userId = JSON.parse(sessionStorage.permission).id;
    await fetch(`http://localhost:3000/twitter/${userId}`, {
        method: "DELETE",
        headers: {
            'Content-Type': 'application/json'
        }
