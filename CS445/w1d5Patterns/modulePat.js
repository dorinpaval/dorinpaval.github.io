...
const shoppingCart=(function() {
    let basket=[];
    return{
        upsertItem: function(prod){
            let aux= basket.find( item => item.id==prod.id);
            if(!aux){
                basket.push(prod);
                //console.log(prod.product.name)
            }
            else{
                    let index= basket.indexOf(aux);
                    basket[index]=prod;
                    console.log(prod.id, prod.product.name)
            }
        },
        ...
        getTotalPrice: function(){
            let sum= basket.reduce( (s,item) => s+(item.product.price*item.count),0);
            return sum;
        },
