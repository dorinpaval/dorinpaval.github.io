...
bank.debit = function (id, amount) {
    ...
    {
        sum += customer.customerTransactions[i];
    }
...
bank.getBalance = function (id) {
    ...
    for(let i=0;i<customer.customerTransactions.length;i++)
    {
        ...
        console.log(customer.customerTransactions[i]);
    }
...
bank.bankBalance = function () {
   let  balance=0;
    //const customer = bank.transactionsDB.find(customer => customer.customerId === key);
    for(let key of bank.transactionsDB){
    for(let i=0;i<key.customerTransactions.length;i++)
    {
        balance += key.customerTransactions[i];
